...
from sqlalchemy import Column, String, Integer, ForeignKey, desc, asc
...
class Check_data_info(Base):
    ...
    def get_last_quality_data(cls, quality_user, task_detail_id, check_user_id):
        ...
        if check_data_info:
            ...
            last_quality_data = Check_data_info.query.filter(Check_data_info.quality_user == quality_user,
                                                             Check_data_info.check_user_id == check_user_id,
                                                             Check_data_info.is_complete == 1,
                                                             Check_data_info.id < check_data_info_id
    ...
    def get_next_quality_data(cls, quality_user, task_detail_id, check_user_id):
        ...
        if check_data_info:
            ...
            next_quality_data = Check_data_info.query.filter(Check_data_info.quality_user == quality_user,
                                                             Check_data_info.check_user_id == check_user_id,
                                                             Check_data_info.is_complete == 1,
                                                             Check_data_info.id > check_data_info_id).order_by(
                asc(Check_data_info.id)).first()
            return next_quality_data
        else:
            return None

    @classmethod
    def get_page_quality_data(cls, quality_user, page, check_user_id):
        page_quality_data = Check_data_info.query.filter_by(quality_user=quality_user, check_user_id=check_user_id,
                                                            is_complete=1).order_by(asc(Check_data_info.id)).slice(page-1, page).first()
        return  page_quality_data

...
class LoginForm(Form):
    # print(type(Form))
    nickname = StringField(validators=[DataRequired(), Length(2,24, message='昵称至少需要2个字符，最多24个字符')])
...
class RegisterForm(Form):
    ...
    realname = StringField(validators=[DataRequired(), Length(2,24, message='姓名至少需要2个字符，最多24个字符')])

    email = StringField(validators=[DataRequired(), Length(6, 64),Email(message='电子邮箱不符合规范')])
